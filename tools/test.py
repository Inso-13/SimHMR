...
def main():
    ...
    # import numpy as np
    # print(cfg.model.body_model_test.joints_regressor)
    # model.body_model_test.joints_regressor = torch.tensor(
    #             np.load(cfg.model.body_model_test.joints_regressor), dtype=torch.float)
    if not distributed:
        if args.device == 'cpu':
            model = model.cpu()
        else:
