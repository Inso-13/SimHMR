...
from mmhuman3d.core.conventions.keypoints_mapping import get_flip_pairs,get_keypoint_idx
...
def get_affine_transform_xy(center,
                            scale,
                            rot,
                            output_size,
                            shift=(0., 0.),
...
class MeshAffineXY:
    ...
    def __call__(self, results):
        ...
        s = results['scale']
        r = results['rotation']
        trans = get_affine_transform_xy(c, s, r, self.image_size)
        inv_trans = get_affine_transform_xy(c, s, 0., self.image_size, inv=True)
        crop_trans = get_affine_transform_xy(c, s, 0., self.image_size)
...
@PIPELINES.register_module()
class KeypointsRandomMask(object):

    def __init__(self, conversion: str = 'smpl_49', rate: float = 0.5) -> None:
        self.conversion = conversion
        self.rate = rate
        
    def __call__(self, results):
        assert results['keypoints2d'].shape[0] == 49
        if results['dataset_name'] == 'coco':
            bad_kpts = ['right_shoulder','left_shoulder','right_hip_extra','left_hip_extra']
        elif results['dataset_name'] == 'mpii':
            bad_kpts = ['right_shoulder','left_shoulder','right_hip_extra','left_hip_extra']
        elif results['dataset_name'] == 'lspet':
            bad_kpts = ['right_hip_extra','left_hip_extra']
        else:
            return results
        bad_index = [get_keypoint_idx(bad_kpt,self.conversion) for bad_kpt in bad_kpts]
        mask_out_index = np.random.choice(bad_index, round(len(bad_index) * self.rate))
        results['keypoints2d'][mask_out_index,2] = 0
